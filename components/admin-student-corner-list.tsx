...
export function AdminStudentCornerList({ items }: { items: any[] }) {
  const [list, setList] = useState(items)

  async function handleDelete(id: any) {
    const supabase = createClient()
    const { error } = await supabase.from("students_corner").delete().eq("id", id)
    if (error) {
          ...
          <CardHeader>
            <div className="flex items-start justify-between">
              ...
              <div className="flex space-x-1">
                <Link href={`/admin/student-corner/${item.id}/edit`}>
